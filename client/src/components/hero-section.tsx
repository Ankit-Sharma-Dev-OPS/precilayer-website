...
const AnimatedCounter = ({
  ...
  return (
    <span
      ref={ref}
      className="text-3xl font-bold text-cyber-400 font-numbers shimmer-text"
...
export default function HeroSection() {
    ...
    if (video && !videoError) {
      const playVideo = async () => {
        try {
          ...
          video.playbackRate = 0.8; // Normal playback speed for smooth performance
          await video.play();
        } catch (error) {
          // Retry on user interaction
          const handleInteraction = () => {
            video.playbackRate = 0.8; // Normal speed on retry
            video.play().catch(() => {});
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-5xl mx-auto px-6 bg-black/50 rounded-lg py-12 shadow-2xl border border-white/10">
        <motion.h1
        ...
        <motion.p
          ...
          className="text-lg md:text-xl mb-8 text-white max-w-4xl mx-auto leading-relaxed"
          style={{ textShadow: "1px 1px 3px rgba(0,0,0,0.8)" }}
          data-testid="hero-description"
        >
          Best-in-class precision machined and additive parts at scale, from
          India to the world.
          <span className="text-cyber-400 font-semibold">
            {" "}
            No MOQs. No inventory.
          </span>{" "}
          Just speed, flexibility, and the future of making.
        </motion.p>
        ...
        >
          <div
            className="scroll-reveal float"
            data-testid="stat-delivery"
          ...
          <div
            className="scroll-reveal float"
            data-testid="stat-minimum-order"
